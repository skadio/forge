...
from forge.vqgraph.vq import VectorQuantize
import dgl
...
class GCN(nn.Module):
    def __init__(
        ...
        self.vq_node = VectorQuantize(dim=input_dim, codebook_size=codebook_size, decay=0.8,commitment_weight=0.25, use_cosine_sim = True)
        self.vq_edge = VectorQuantize(dim=input_dim, codebook_size=codebook_size, decay=0.8,commitment_weight=0.25, use_cosine_sim = True)
    ...
    def forward(self, g, feats):
        ...
        quantized_edge = self.decoder_1(quantized_edge)
        quantized_node = self.decoder_2(quantized_node)
...
class SAGE(nn.Module):
    ...
    def inference(self, dataloader, feats):
        """
        Inference with the GraphSAGE models on full neighbors (i.e. without neighbor sampling).
