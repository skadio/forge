...
"""
Train student MLP models with Token-Based GNN-MLP Distillation.
"""
...
def run_transductive(
    ...
    acc = evaluator(out[idx_test], labels[idx_test])
    logger.info(
        f"Best valid models at epoch: {best_epoch: 3d}, acc: {acc :.4f}"
    )
...
def run_inductive(
    ...
    The models is trained on the observed graph `obs_g`, and evaluated on both the observed test nodes (`obs_idx_test`) and inductive test nodes (`idx_test_ind`).
    The input graph is assumed to be large. Thus, SAGE is used for GNNs, mini-batch is used for MLPs.
    ...
    out[idx_obs] = obs_out
    logger.info(
        f"Best valid models at epoch: {best_epoch :3d}, acc_tran: {acc_tran :.4f}, acc_ind: {acc_ind :.4f}"
    )
...
def distill_run_transductive(
    ...
    The input graph is assumed to be large, and MLP is assumed to be the student models. Thus, node feature only and mini-batch is used.

    out_t: Soft labels produced by the teacher models.
