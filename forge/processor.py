...
class DataProcessor:
    ...
    def convert_mip_to_mipinfo(self, input_folder, output_file, perturb_list=None, has_return=False) \
        ...
        for idx in tqdm(range(len(sorted_instances))):
            ...
            mip_info = self.generate_mip_info(mip_model, has_graph_features=False)
    ...
    @staticmethod
    def generate_mip_info(mip_model, is_debug=False, has_graph_features=True):
        ...
        if is_debug:
            ...
            s = time.time()
        dgl = get_dgl_graph(sp.csr_matrix(adj), features, sp.csr_matrix(coeff_adj))

        if dgl is False:
            if is_debug:
                print("DGL returned false, MIPInfo is empty")
            return MIPInfo()
